...
def game_ronaldo():
    def outro():
        ...
        game_over_window.geometry(f"{WIDTH}x{HEIGHT}+{math.floor((screenwidth / 2) - (WIDTH / 2))}+{math.floor((screenheight / 2) - (HEIGHT / 2) - 10)}")
        ...
        frame2 = Frame(game_over_window, bg='#032750')
        frame2.pack(pady=20)
        restart = Button(frame2, text='Try Again', bg='#cf8af4', fg='#032750', font=('Segoe Print', 18), command=restartF)
        restart.pack(side=LEFT)
        close = Button(frame2, text='Close', bg='#cf8af4', fg='#032750', font=('Segoe Print', 18), command=close)
    ...
    def dragmo(event):
        ...
        widget.place(x=x, y=500 - player_height)
    ...
    window.bind("<Button-1>", dragst)
    window.bind("<B1-Motion>", dragmo)
